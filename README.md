...
> **Two implementations.** The PowerShell variant documented below is the stable reference, now **frozen to bugfixes** — new work happens in the TypeScript port in [`ts/`](./ts/), which is **feature-complete**: the engine, all log tools (byte-identical output, cross-checked), subagents, MCP/skills/hooks/background tasks/web, an Ink terminal UI, and a headless mode (`-p` + `--continue` + JSON output, plus an importable `SenseiAgent` API). Both share `~/.sensei`. Feature map: [`ts/PARITY.md`](./ts/PARITY.md).
