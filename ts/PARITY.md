...
| task / verify | ✅ | ✅ | in-process depth-1 loops, subagent tools excluded |
| task_parallel | ✅ | ✅ | Promise.all over ≤3 in-process subagents (ThreadJob re-bootstrap deleted); children are non-interactive |
| exit_plan_mode | ✅ | ✅ | approval via the host callback; ends plan mode + re-seeds the system prompt |
...
| Summarizing compaction | ✅ | ✅ | 80% budget trigger, user-boundary cut, 25% tail, trim fallback; /compact forces |
...
| auto_verify | ✅ | ✅ | verifier subagent after write-tool turns |
| Plan mode gate + exit_plan_mode | ✅ | ✅ | |
...
| Slash commands | ✅ | ✅ | full set: /help /clear /compact /plan /style /color /model /config /mcp /permissions /skills /newskill /tasks /todos /cost /memory /init /investigate /resume /exit /quit + custom .md commands + /<skillname> |
...
| Subagents / exit_plan_mode / auto_verify / compaction | ~15 | 13 (subagents.test.ts) |

**Port complete.** All PS-variant features are ported or explicitly marked ➖; the TS variant additionally has the headless mode, session envelopes, `--allow`/`--file`, and the structured event stream. Total: 106 vitest tests vs 149 PS smoke assertions (same behavioral coverage, plus TS-only features).
